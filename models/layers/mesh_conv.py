...
class MeshConv(nn.Module):
    ...
    def forward(self, x, mesh):
        x = x.squeeze(-1)# this is some dimention squeezing thing happening 
        #https://pytorch.org/docs/stable/torch.html?highlight=cat#torch.cat
        G = torch.cat([self.pad_gemm(i, x.shape[2], x.device) for i in mesh], 0) # here the 0 is for the axis of concat
    ...
    def create_GeMM(self, x, Gi):
        ...
        #
        odim = x.shape
        # permute means altering someting . looks like transpose kind of thing is happening
        #>>> x = torch.randn(2, 3, 5)
        #>>> x.size()
        #torch.Size([2, 3, 5])
        ...
        # contiguous some kind of copy i guess Returns a contiguous tensor containing the same data as self tensor.
